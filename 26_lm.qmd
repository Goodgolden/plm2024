...
The final model is only fitted with `mgcv` or `gamlss`; 
We cannot use a linear mixed model for the testing dataset prediction.
The prediction for the linear mixed model has to be conditional on the random effects.
