...
The formula is the part affecting the performance a lot. 
Here are the formula used for the old version of PLM:
...
Stef used the lme4::lmer() function, which is faster and more reliable with complicated random effects.
However the JMbayes package can only take nlme:lme() function for prediction by Drizopoulos.
...
Here we can use more than the first time visit value, how about we use the first 10 days.
If the person does not have any visit in ten days we just ignore that patient.
Probably use EPIC data or other data with more observations at the beginning.
...
The results are not good, the prediction is not good at all.
